...
const Calendar = ({ config, courses }: CalendarProps) => {
  ...
  return (
    <div className="w-full h-full flex flex-col">
      <Card
        id="calendar"
        className="w-full h-full flex flex-col justify-center items-center bg-white p-4"
      >
        ...
        <CardContent className="w-full h-full flex flex-row overflow-hidden">
          {showHours && (
            <div
              id="timecol"
              className="w-1/8 h-full flex flex-col px-2 divide-y-[1px] divide-y-slate-50"
            >
              ...
              <div
                id="hours-flex"
                className="h-full flex flex-col py-2 divide-y-[1px] divide-y-slate-50"
              >
                {hours.map((e) => (
                  <div
                    key={e}
          ...
          <div
            id="cal"
            className={`px-4 w-full h-full grid ${gridLayouts[numberOfDays]} divide-x-[1px] divide-x-slate-50`}
          >
            {displayDays.map((dayName: string, dayIdx: number) =>
              DayComponent(dayName, dayIdx + startDay - 1, courses, {
                startHour,
                endHour,
                startDay,
                numberOfDays,
